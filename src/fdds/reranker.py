...
class LLMReranker(Reranker):
    ...
    Only respond with 'Yes' if the document contains information
    that directly helps answer the query.
    If the query is a greeting, off-topic, or the document
    does not contain relevant information, respond with 'No'.
    ...
    async def rerank(
        ...
        accepted_results = [element for element in scoring_results if element[1] > 0.0]
        return [elem for elem, _ in accepted_results[: reranker_options.top_n]]

    async def _score_elements(
        self, elements: Sequence[Element], query: str
    ) -> Sequence[float]:
        """
        ...
        for doc in elements:
            full_prompt = self.prompt_template.format(
                query=query, document=doc.text_representation
            )
            ...
            prob = math.exp(logprob) if answer == "Yes" else 0
